...
两次编排运行的 scorecard 对比——拓扑切换后，此前掉分的维度回升：

| 维度 | map-reduce（08-21） | critique（08-22） |
|---|---|---|
| correctness | 4 | 4（稳定测量下为 5，见 §3.2） |
| hallucination | **4** | **5**（假设显式标注生效） |
| technical_depth | **4** | **5**（深度保留：fencing/epoch、Outbox 乱序、读己之写等） |
| 其余三维 | 5 | 5 |
